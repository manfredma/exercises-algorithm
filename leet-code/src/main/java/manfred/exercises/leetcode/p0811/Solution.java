package manfred.exercises.leetcode.p0811;
...
class Solution {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cpdomain : cpdomains) {
            String[] count2domain = cpdomain.split(" ");
            int count = Integer.parseInt(count2domain[0]);
            String domain = count2domain[1];
            String[] levelDomains = domain.split("\\.");
            String suffix = "";
            for (int i = levelDomains.length - 1; i >= 0; i--) {
                String levelDomain = levelDomains[i] + suffix;
                if (!domainToCount.containsKey(levelDomain)) {
                    domainToCount.put(levelDomain, 0);
                }
                domainToCount.put(levelDomain, domainToCount.get(levelDomain) + count);
                suffix = "." + levelDomain;
            }
        }
        List<String> result = new ArrayList<>();
        domainToCount.forEach(
                (x, y) -> {
                    result.add(y + " " + x);
                }
        );
