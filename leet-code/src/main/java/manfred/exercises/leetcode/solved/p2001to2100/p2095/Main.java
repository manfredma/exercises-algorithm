...
1 <= Node.val <= 10^5
*/
/** LeetCode 第 2095 题的测试入口。 */
public class Main {
    public static void main(String[] args) {
        Solution s = new Solution();
        assertArrayEquals(
                new int[] {1, 3, 4, 1, 2, 6}, values(s.deleteMiddle(list(1, 3, 4, 7, 1, 2, 6))));
        assertArrayEquals(new int[] {1, 2, 4}, values(s.deleteMiddle(list(1, 2, 3, 4))));
        assertArrayEquals(new int[] {2}, values(s.deleteMiddle(list(2, 1))));
        assertArrayEquals(new int[] {}, values(s.deleteMiddle(list(1))));
    }
