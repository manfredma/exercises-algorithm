...
4. **写 `readme.md`** — 按 content-spec「README 与题面注释规范」「图片规范」生成；图片下载到 `wip/pXXXX/images/`。
5. **写 `Main.java`** — 必须有 `public static void main(String[] args)`，将全部官方示例写为可执行断言，并按题型补充边界、空/极值、并列结果和典型结构等必要用例。`main` 最后一句打印 `leet#XXXX passed`（`XXXX` 为 4 位题号，如第 1 题打印 `leet#0001 passed`）。所有测试放在 `Main.main`，**不得新增 JUnit/TestNG 测试类**，不得在 `src/main/java` 用 `@Test`。
